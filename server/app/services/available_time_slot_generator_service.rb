class AvailableTimeSlotGeneratorService
  ...
  def initialize(date, duration)
    ...
    @duration = duration
    @unavailable_slots = BookedSlot.where(date: date).order(start_date: :asc, end_date: :asc).pluck(:start_date, :end_date)
  end
  ...
  def get_daily_slot
    ...
    remainder = (duration.to_f / INTERVAL.to_f).ceil
    slots[0..-remainder]
  end

  # get unavailable/booked slots
  def get_booked_slots
    booked_slots = BookedSlot.where("DATE(start_date) = ? OR DATE(end_date) = ? ", date, date).order(start_date: :asc, end_date: :asc).pluck(:start_date, :end_date)

    return booked_slots.empty? ? [] : flatten_overlapping_slots(booked_slots)
  end
  ...
  def get_timeframe_slots
    ...
    return get_daily_slot if unavailable_slots.empty?
